...
import type { DebtInstallmentInput } from '../lib/types'
...
type PreviewRow = DebtInstallmentInput & { text: string }

const now = new Date()

type Props = { onClose: () => void }

export function DebtForm({ onClose }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await qc.invalidateQueries({ queryKey: ['debtOverview'] })
      onClose()
    } catch (e) {
    ...
    >
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-bold">Nova dívida</h2>
        ...
        <input
          ...
          placeholder="Descrição (ex: Empréstimo, carnê…)"
