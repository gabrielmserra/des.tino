...
class Dashboard(ctk.CTkScrollableFrame):
    ...
    def _compute_card_warning(self, overview: list, s: dict) -> str:
        ...
        try:
            ...
            if commitments:
                cur_total = float(commitments[0].get("card_total") or 0)
                if cur_total > 300:
                    parts.append(f"fatura em aberto: {format_currency(cur_total)}")
            if len(commitments) > 1:
                next_total = float(commitments[1].get("card_total") or 0)
                if next_total > 300:
    ...
    def _draw_credit_panel(self, cards: list, s: dict,
        ...
        for i, card in enumerate(cards):
            ...
            # "Fecha" (próximo fechamento) e "Vence" (fatura já fechada,
            # aguardando pagamento) são sobre ciclos diferentes — nunca
            # mostrados juntos, senão fica parecendo contraditório.
            if unpaid > 0:
